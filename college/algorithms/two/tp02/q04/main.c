...
bool binary_search(Show *shows, int left, int right, char *target) {
    while (left <= right) {
        int mid = left + (right - left) / 2;
        int cmp_result = cmp(shows[mid].title, target);
        comparacoes++;
        if (cmp_result == 0) {
            return true;
        }
...
int main() {
    ...
    Show *readed_shows = malloc(MAX_SHOWS * sizeof(Show));
    int inserted_count = 0;
    char input[MAX_STR_SIZE];
    char search_title[MAX_STR_SIZE];
    bool exists = false;
    ...
    if (fgets(input, sizeof(input), stdin) != NULL) {
        size_t lenInput = len(input);
        if (lenInput > 0 && input[lenInput - 1] == '\n')
            input[lenInput - 1] = '\0';
        while (cmp(input, "FIM") != 0) {
            Show *s = findOne(input, shows, show_count);
            if (s != NULL) {
                readed_shows[inserted_count++] = *s;
            }
    ...
    order_by_title(readed_shows, inserted_count);

    if (fgets(search_title, sizeof(search_title), stdin) != NULL) {
        int lenSearch = len(search_title);
        if (lenSearch > 0 && search_title[lenSearch - 1] == '\n')
            search_title[lenSearch - 1] = '\0';
        while (cmp(search_title, "FIM") != 0) {
            exists = binary_search(readed_shows, 0, inserted_count - 1,
                                   search_title);
            if (exists) {
                printf("SIM\n");
            } else {
    ...
    FILE *log = fopen(MATRICULA "_binaria.txt", "w");
    if (log) {
        fprintf(log, "%s\tTempo:%f.s\tComparações: %ld", MATRICULA, elapsed,
                comparacoes);
        fclose(log);
    }
    free(shows);
