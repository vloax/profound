...
void insertion_sort_partial(Show *arr, int n, int k) {
    int limit = (k < n) ? k : n;
    for (int i = 1; i < n; i++) {
        Show key = arr[i];
        int j = i - 1;
        ...
        while (j >= 0) {
            comparacoes++;
            int cmp_type = cmp_ignore_case(arr[j].type, key.type);
            int cmp_title = cmp_ignore_case(arr[j].title, key.title);
...
int main() {
    ...
    clock_t start = clock();
    insertion_sort_partial(saved, saved_count, 10);
    clock_t end = clock();
    ...
    FILE *log = fopen(MATRICULA "_insercao_parcial.txt", "w");
    if (log) {
        fprintf(log,
