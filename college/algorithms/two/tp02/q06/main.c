...
void selectionSortRec(Show *arr, int start, int n) {
    if (start >= n - 1) return;
    int minIndex = start;
    for (int j = start + 1; j < n; j++) {
        comparacoes++;
        if (cmp_ignore_case(arr[j].title, arr[minIndex].title) < 0) {
            minIndex = j;
        }
    }
    if (minIndex != start) {
...
int main() {
    ...
    clock_t start = clock();
    selectionSortRec(saved, 0, saved_count);
    clock_t end = clock();
    ...
    FILE *log = fopen(MATRICULA "_selecao.txt", "w");
    if (log) {
        fprintf(log, "%s\tComparações: %ld\tMovimentações: %ld\tTempo: %.3f\n",
