...
int compare_shows(Show a, Show b) {
    comparacoes++;

    char *director_a =
        (a.director == NULL || a.director[0] == '\0') ? "NaN" : a.director;
    char *director_b =
        (b.director == NULL || b.director[0] == '\0') ? "NaN" : b.director;

    int cmp_type = cmp_ignore_case(director_a, director_b);

    if (cmp_type == 0) {
        char *title_a =
            (a.title == NULL || a.title[0] == '\0') ? "NaN" : a.title;
        char *title_b =
            (b.title == NULL || b.title[0] == '\0') ? "NaN" : b.title;
        comparacoes++;
        return cmp_ignore_case(title_a, title_b);
    }
...
void min_heapify(Show *arr, int n, int i) {
    int smallest = i;
    int left = 2 * i + 1;
    int right = 2 * i + 2;

    if (left < n && compare_shows(arr[left], arr[smallest]) < 0)
        smallest = left;

    if (right < n && compare_shows(arr[right], arr[smallest]) < 0)
        smallest = right;
...
void heapsort_parcial(Show *arr, int n, int k) {
    if (k > n) k = n;
    ...
    // Array temporário para armazenar os k menores elementos
    Show *temp = (Show *)malloc(k * sizeof(Show));
    if (!temp) return;

    for (int i = 0; i < k; i++) {
        temp[i] = arr[0];                // Extrai o menor da raiz
        arr[0] = arr[n - 1 - i];         // Substitui pela última folha
        min_heapify(arr, n - 1 - i, 0);  // Corrige o heap
        movimentacoes++;
    }

    // Ordena os k menores elementos (insertion sort)
    for (int i = 1; i < k; i++) {
        Show key = temp[i];
        int j = i - 1;
        while (j >= 0 && compare_shows(temp[j], key) > 0) {
            temp[j + 1] = temp[j];
            j--;
            movimentacoes++;
        }
    ...
    // Copia de volta os k elementos ordenados
    for (int i = 0; i < k; i++) {
        arr[i] = temp[i];
        movimentacoes++;
    }
...
int main() {
    ...
    clock_t start = clock();
    // Aplica o heapsort parcial para ordenar os 10 primeiros elementos
    heapsort_parcial(saved, saved_count, 10);
    clock_t end = clock();

    // Imprime apenas os k primeiros elementos já ordenados
    int toPrint = saved_count < 10 ? saved_count : 10;
