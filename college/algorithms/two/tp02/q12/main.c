...
void bubblesort(Show *arr, int n) {
    for (int i = 0; i < n - 1; i++) {
        for (int j = 0; j < n - i - 1; j++) {
            comparacoes++;
            int cmp_date =
                compare_date(arr[j].date_added, arr[j + 1].date_added);
            int cmp_title = cmp_ignore_case(arr[j].title, arr[j + 1].title);

            bool troca = false;
            if (cmp_date > 0) {
                troca = true;
            } else if (cmp_date == 0 && cmp_title > 0) {
                troca = true;
            }
...
int main() {
    ...
    FILE *log = fopen(MATRICULA "_bolha.txt", "w");
    if (log) {
        fprintf(log,
