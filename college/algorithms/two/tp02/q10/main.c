...
int compare_shows_date_then_title(Show a, Show b) {
    comparacoes++;
    int cmp_date = compare_date(a.date_added, b.date_added);
    if (cmp_date != 0) return cmp_date;
    return cmp_ignore_case(a.title, b.title);
}
...
int partition(Show *arr, int low, int high) {
    ...
    for (int j = low; j < high; j++) {
        if (compare_shows_date_then_title(arr[j], pivot) <= 0) {
            i++;
            Show temp = arr[i];
            arr[i] = arr[j];
            arr[j] = temp;
...
int main() {
    ...
    clock_t start = clock();
    quicksort(saved, 0, saved_count - 1);
    clock_t end = clock();
    ...
    FILE *log = fopen(MATRICULA "_quicksort.txt", "w");
    if (log) {
        fprintf(log,
