...
/**
 * Retorna o maior release_year no array.
 */
int get_max_release_year(Show *arr, int n) {
...
int get_max_title_length(Show *arr, int n) {
    int max_len = 0;
    for (int i = 0; i < n; i++) {
        int l = len(arr[i].title);
...
/**
 * Counting sort estável baseado no dígito definido por exp (1, 10, 100, ...).
 * Atualiza a métrica de movimentações em 'movimentacoes'.
 */
void counting_sort_by_digit(Show *arr, int n, int exp) {
    Show *output = malloc(n * sizeof(Show));
    int count[10] = {0};

    // Contagem de ocorrências do dígito
    for (int i = 0; i < n; i++) {
        int digit = (arr[i].release_year / exp) % 10;
        count[digit]++;
        movimentacoes++;
    }
    ...
    // Monta o array de saída de trás para frente (estável)
    for (int i = n - 1; i >= 0; i--) {
        int digit = (arr[i].release_year / exp) % 10;
...
/**
 * Algoritmo Radix Sort (LSD) para ordenar por release_year.
 */
void radix_sort_by_release_year(Show *arr, int n) {
    int max = get_max_release_year(arr, n);
    for (int exp = 1; max / exp > 0; exp *= 10) {
        counting_sort_by_digit(arr, n, exp);
    }
}

void counting_sort_by_char(Show *arr, int n, int pos) {
    Show *output = malloc(n * sizeof(Show));
    int count[258] = {0};  // ASCII 0-255 + 1 extra

    for (int i = 0; i < n; i++) {
        char c = (len(arr[i].title) > pos) ? tolower(arr[i].title[pos]) : 0;
        count[(int)c + 1]++;
        movimentacoes++;
    }
...
void radix_sort_by_title(Show *arr, int n) {
    int max_len = get_max_title_length(arr, n);
    for (int pos = max_len - 1; pos >= 0; pos--) {
        counting_sort_by_char(arr, n, pos);
    }
}

void radixSort(Show *arr, int n) {
    radix_sort_by_title(arr, n);         // critério de desempate
    radix_sort_by_release_year(arr, n);  // chave principal
}
...
int main() {
    ...
    FILE *log = fopen(MATRICULA "_radixsort.txt", "w");
    if (log) {
        fprintf(log,
